...
## Plan-Phase Follow-up Round (2026-05-17, after RESEARCH.md returned)

### Q10: Wallets Currency cell — editable or read-only? (resolves Open Q1: WALT-04 immutability vs D-PH5-W5 "click any cell becomes editable")

**User choice:** Editable, server validates.

**Locked as D-PH5-W12:** WALT-04 rescinded for Phase 5. Currency cell inline-editable. Server still rejects non-budget-currency on RESERVE wallets (D-PH5-R3 unchanged). Amount stays numerically same on currency change — no FX conversion at the cell.

### Q11: Reserves tab UI/UX + which categories included?

**Claude explanation given:** All active categories auto-included from `category_reserve_balance` VIEW. 4-column table: Category / Reserve balance / Wallet share / Actions. Totals row at bottom shows Σ vs Σ vs mismatch.

**User pushed back with 4 new requirements:**

1. **Global reserves toggle in Settings** — when disabled: reserves hidden everywhere, no calculations.
2. **Per-category opt-out** — drag-between-sections Active / Excluded, same primitive as Wallets.
3. **Manual rebalance** — user can change a category's reserve number directly (e.g., "Housing has 5000, I want 4000"). Supports: initial seeding from existing reserves, fixing mis-planned categories, withdrawing excess, transferring between categories. Auto-calculations still work.
4. **Mismatch indicators** — over/under signal at totals + per-category highlight when underfunded.

### Q12: How to absorb the 4 new requirements?

**User choice:** Absorb everything into Phase 5 (one mega-phase).

**Implication:** Phase 5 grows from ~5 plans to ~9–11 plans. New migration (adjustments table + 2 column adds + VIEW rewrite). Decisions extended (D-PH5-R7..R13, D-PH5-W12).

### Q13: Withdraw model — destination wallet required, or pure adjustment?

**User choice:** Pure adjustment. Quote: _"category housing had 5000 reserves, now I just change it to 4000 and later I can either reduce reserve wallet with 1000 or move that 1000 into one or multiple other reserve categories. Until nothing of it is done — it's landed in that special bucket where we show that we have less reserves than expected or more"_.

**Locked as D-PH5-R7 + D-PH5-R13:** Inline-edit cell writes adjustment row. No transfer modal. Transfer = two cell edits. Mismatch bucket reflects in-flight delta as visible truth signal.

### Q14: Rebalance UX — inline-edit, modal buttons, or both?

**User choice:** Inline-edit (recommended option). Confirms D-PH5-R7.

### Q15: When a category moves to "Excluded" — what happens to its accumulated reserve?

**Not directly answered.** Defaulted to **Frozen** (recommended): reserve + adjustments preserved, hidden from math, restorable by drag back to Active. Locked as D-PH5-R10. User invited to override; no override given.

### Q16: Where does the reserves on/off toggle live in Phase 5?

**Not directly answered.** Defaulted to **Schema only in Phase 5; toggle UI in Phase 6 Settings**. Phase 5 adds column + cascading hide behavior. Locked as D-PH5-R11.

### Q17: Reserves tab mismatch totals row — sticky or static?

**User choice:** Sticky-bottom always (both desktop + mobile). Locked as D-PH5-R12.
