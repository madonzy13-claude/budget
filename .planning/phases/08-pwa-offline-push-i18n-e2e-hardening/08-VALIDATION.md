...
| Offline read + "never cached" bar on device | PWAX-02     | Playwright `setOffline` does NOT block the SW's API fetches, so offline no-data states can't be E2E-verified (2026-06-17) | On a real device: open a budget online (warms tabs) → go offline → reopen + navigate tabs → cached data renders with "data updated X ago"; open a budget NEVER opened online → bar reads "data never cached"; nav cache-miss shows offline-shell **Back** button (no black screen) |

_Automated layers (SW registration, offline write rollback-toast, registry fire, namespace completeness, deep-link routing) are covered by component/integration/E2E tests; only the above need a human. (The offline write-queue/replay was removed 2026-06-16/17 — see the 08-CONTEXT.md banner.)_
