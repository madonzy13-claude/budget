...
- Per-workspace single-workspace UI areas: amounts render in that workspace's `default_currency` — **no FX badge needed** because the source currency *is* the display currency.
...
- Numbers always with the currency code or % suffix ("100%", "USD 1,234.50") — currency code goes *before* the amount in EN, *after* in PL and UK (already next-intl's default ICU `{value, number, ::currency/USD}` behavior; do not hand-format).
...
- Submit-time server errors: shadcn `Alert` (variant=destructive) inside the card, above the form fields. Toast for *success*, in-card alert for *error* (so the error stays put while the user reads it).
...
- Dashboard widget *content* (Phase 1 ships layout + skeletons only)
