...
- truth: "Investments DnD: a COLLAPSED group shows the child indent ONLY while the item is clearly BELOW the header; once the item reaches/covers the header (or rises above it) the indent clears and a drop lands the item above the group."
  status: fixed
  reason: "User retest: collapsed below=child now works, BUT dragging the item up so it COVERS the header still showed the indent and a drop there joined the group — they read that overlap as 'above the group' and want no indent + land-above."
  severity: minor
  fix: "The collapsed band still started at the header CENTRE, which is exactly where the list strategy swaps the dragged row across the header — so the band covered the header itself and an item merely overlapping it still counted as a child. Start the COLLAPSED band at the header's BOTTOM instead (band = [headerBottom, headerBottom+64]); the indent now shows only while the item is clearly below the header (a real gap) and clears the instant it reaches/covers it. EXPANDED groups keep the header-centre start — that's the #2 first-child-from-above zone (members make the boundary unambiguous), and it's unchanged."
  commit: 0968fe5
  verified: "tsc clean; 103 vitest green. Live Playwright 390px, group VERIFIED collapsed: per-step — dragging the item up, indent ON only while the item is clearly below the header (tTop 757, gap below the 693–749 header) and OFF the moment it overlaps the header (tTop 731) and all the way above; drop while clearly-below → JOINS as child, drop while covering/above → loose above the group (DB-confirmed both). #2 expanded first-child path unchanged."

- truth: "Investments DnD: dragging an item OUT of (above) its own group clears the child indent the instant it reaches the header; and dropping any item never makes the row blink out (no flicker)."
  status: fixed
  reason: "User retest: (a) the same overlap problem on the way OUT — an item dragged up out of an EXPANDED group still showed the indent while it covered the header; (b) on every drop the row disappears for a fraction of a second then reappears (flicker)."
  severity: minor
  fix: "(a) The expanded join band started at the header CENTRE for ALL groups. Made the band ORIGIN-aware: the group the dragged item already belongs to (ejecting) starts at the header BOTTOM — so the indent clears the instant the item rises to cover the header — while a group it is JOINING from outside keeps the header-centre start (the #2 first-child-from-above zone). activeGroup is captured in handleDragStart and threaded into snapshotDragGeometry. (b) The render mapped each group to a <Fragment> with its holdings nested inside; a <Fragment> is a reconciliation boundary, so a holding changing group on drop unmounted at the old spot and remounted at the new one (the blink). Flattened the render with flatMap so EVERY header and holding is a direct child of the one container, keyed by id → React MOVES the row instead of remounting."
  commit: fc67066
  verified: "tsc clean; 103 vitest green. Live Playwright 390px: (a) expanded Tech, drag MSFT up out — per-step the indent is ON only while MSFT is below the header and clears exactly at headerBottom (685), before it covers the header; drop above → loose (DB). (b) drag Tesla (loose) into Tech (real group change, DB: Tesla→Tech) and tag the row's DOM node — after the drop + optimistic re-render the SAME node persists (document.contains=true, ===node=true, custom tag survived) → no unmount/remount → no flicker. #2 join-from-above + eject + collapsed all still correct."

- truth: "Investments DnD: a normal item dragged from ABOVE into an expanded group shows the child indent — and drops first-in-group — the instant it sits below the header (no dead zone, no jump-back); and dropping any item never blinks the row out (flicker)."
  status: fixed
  reason: "User retest of the prior fc67066 batch: (1) the first-child-from-above STILL had a dead zone — the item could sit visually below the group header WITHOUT indent, and a drop there jumped it back above; only dragging a few px lower turned the indent on and dropped it first. User's rule: 'if the item is below the group header and inside the group it must have indent and dropping there must always drop inside.' (2) flicker on drop persisted despite the flatMap/no-remount fix."
  severity: minor
  fix: "(1) ROOT: @dnd-kit's verticalListSortingStrategy swaps the header UP over the dragged row the instant `over` flips to it — which (closest-centre, equal-height rows) happens at the MIDDLE OF THE GAP above the header (headerTop − gap/2), NOT at the header centre. The join band's top was snapshotted at headerCentre (~28px below the swap), so in [swap, centre] the row rendered below the header yet aMid < centre → no indent, and the drop resolved loose → jump-back. Moved the joining band's TOP to headerTop − gap/2 (the swap point) so the indent turns on the exact frame the row clears the header and the drop lands first-in-group. Extracted the band maths to a pure, unit-tested computeJoinBands(geoms, activeGroup) (ejecting/collapsed groups still start at headerBottom — origin-aware, unchanged). (2) The flatMap fix removed the remount but NOT the blink — the real cause was the always-mounted <DragOverlay>: with an overlay present, @dnd-kit runs its default drop animation that pins the SOURCE row at opacity:0 for ~250ms while it flies the overlay home — even for a holding drag whose overlay child is null. `committed` already places the row instantly, so the animation is pure downside → dropAnimation={null}. Also hardened the `committed` clear: a group-change drop fires TWO optimistic mutations (reorder + group) on the same key; clearing committed on the FIRST holdings change painted the half-applied state (reordered, old group) for a frame — now it clears only once live data matches committed (order + group)."
  commit: efef628
  verified: "tsc clean; 108 investments vitest green (5 new computeJoinBands cases). Live Playwright 390px, DB-verified: (1) Tesla loose-above EXPANDED Tech, dragged down — per-frame the state flips atomically from above-header/no-indent straight to below-header/indented (no intermediate below-without-indent frame); drop in the just-below-header zone → Tesla group=Tech sort 0 (FIRST, before Microsoft). (2) frame-captured the dropped row's opacity over 24 post-pointerup frames — pre-fix it read opacity 0 on frames 1–15 (inline `opacity:0; transition:transform linear` from the overlay drop animation), post-fix it stays 1 on every frame. Regressions clean: group-block drag of Tech still cohesive + header opacity restored to 1 (lands below Metals/Apple, members intact); eject-up of Microsoft clears the indent before it rises above the header (no bad frame) and lands loose."
  user_confirmed: "Pass (2026-06-28) — first-child-from-above + drop flicker both confirmed fixed on device."
