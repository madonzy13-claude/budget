...
4. Every money value is multiplied by one uniform factor **per budget pair**, so sums, limits, reserve balances and FX conversions stay arithmetically consistent within each budget. The personal demo budget is **USD** (PLN relabeled, other currencies preserved); the family demo budget stays **PLN**. The demo user's `display_currency` is USD, so the two-currency pair is what the aggregate converts across.
