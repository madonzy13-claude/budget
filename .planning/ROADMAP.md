...
- [ ] **Phase 11: Budget Overview Tab** _(v1.2 track)_ — New first BDP pill (`overview`): 5 always-visible summary cards (available-to-spend, capitalization=net-worth, this-month overspent categories, cushion real-months+amount, available reserves) + three collapsible time-range chart sections (Planned vs Real with category selector · Overspent · Reserves). All metrics **compute-on-read from the append-only ledger** (no snapshot table, no cascade) cached via the app's persisted React-Query SWR. Introduces the chart lib (recharts). Archived categories kept in historical stats, dropped from current cards. Financial-Wealth section is Phase 12. TDD, EN/PL/UK, E2E Gherkin
- [ ] **Phase 12: Overview — Financial Wealth Section** _(v1.2 track)_ — Adds the Financial-Wealth section to the Overview tab: capitalization-vs-investments toggle (default capitalization), grow/loss + invested + monthly-avg-grow stats over range, capitalization/investment time-series + month-over-month dynamics charts, and (investments view) a per-type pie reusing Phase 9 type colors. Built on persisted snapshots (past prices unrecoverable) reusing/extending Phase 9 `instrument_price_snapshots`. **Depends on Phase 9 investments web UI shipping.** TDD, EN/PL/UK, E2E Gherkin
...
| 10. User Settings Redesign _(v1.2)_         | 6/6            | UAT done    | 2026-06-28 |
| 11. Budget Overview Tab _(v1.2)_            | 0/TBD          | Context'd   | -          |
| 12. Overview — Financial Wealth _(v1.2)_    | 0/TBD          | Planned     | -          |
...
**Goal:** Add a new first BDP pill, `overview`, that gives the family a budget-wide snapshot — 5 always-visible summary cards plus three collapsible, time-range-scoped chart sections (Planned · Overspent · Reserves). Every metric is reconstructed on-read from the append-only ledger + SCD-2 limits and cached via the existing persisted React-Query SWR, so the tab paints instantly from cache, revalidates in the background, and self-corrects when any past data changes (mutation → invalidate). No new snapshot table, no dependency-cascade machinery. The Financial-Wealth section is split out to Phase 12.
**Depends on:** Phase 3 (BDP pill/carousel/prefetch frame) · Phase 4 (spendings-summary) · Phase 5 (reserves view + cushion summary)
...
2. Five summary cards render, each FX-converted to budget currency: available-to-spend (Σ SPENDINGS wallets), capitalization (Σ all wallets = net worth), this-month overspent categories, cushion (real months + total amount from `get-cushion-summary`), available reserves (Σ RESERVE wallets).
3. Three sections (Planned · Overspent · Reserves), each collapsed by default, render charts scoped to a shared range selector (this month · last 3 months · this year · all · custom from→to); buckets are monthly by default, daily for `this month` / short custom spans.
4. Planned section: a category selector (default = none) toggles between budget-wide and per-category Planned-vs-Real timeline; plus planned-avg-vs-real-avg (Y=category), planned-recurring-per-month, and planned-recurring-per-category charts.
5. Overspent section shows total overspent in range + an overspent-by-category bar; Reserves section shows a reserves-by-category bar.
...
7. All metrics compute-on-read (no snapshot table); the tab renders cached data instantly and revalidates in background; editing a past transaction is reflected after invalidation with correct recomputed history.
8. recharts (latest stable) added; chart components are responsive + themed per DESIGN.md; all strings EN/PL/UK; E2E Gherkin covers the golden overview render + range switch + category selector.

### Phase 12: Overview — Financial Wealth Section _(v1.2 track)_

**Goal:** Extend the Overview tab with the Financial-Wealth section: a capitalization-vs-investments toggle (default capitalization), over-range stats (grow/loss amount + %, invested, monthly-average grow %), a capitalization/investment value time-series, a month-over-month dynamics (%) chart, and — in the investments view — a per-type pie reusing Phase 9 type colors. Unlike Phase 11, value-over-time can't be reconstructed from current holdings (past prices/quantities are lost), so this section reads persisted snapshots, reusing/extending Phase 9's `instrument_price_snapshots` (+ a snapshot-cadence decision for the "≤3h ticks / last-3h live" requirement).
**Depends on:** Phase 11 (Overview frame + sections) · **Phase 9 investments web UI shipping** (holdings + price cache live)
**Requirements**: OVW-WEALTH-\* (enumerated at plan time)

**Success Criteria** (what must be TRUE):

1. The Financial-Wealth section renders inside the Overview tab with a capitalization/investments toggle defaulting to capitalization; hidden or empty-stated when the INVESTMENTS feature flag is off / no holdings.
2. Stats show grow/loss (amount + %), invested, and monthly-average grow (%) over the selected range, FX-converted to budget currency.
3. A value time-series chart and a month-over-month dynamics (%) chart render from persisted snapshots; recent value is live-computed from the price cache so the latest point is current.
4. Investments view adds a per-type pie using the same type colors as Phase 9; tap (mobile) / hover (desktop) reveals the share % + type label.
5. Snapshot cadence honors the "ticks no rarer than every 3h, last 3h always live" requirement (cadence + live-tail decided at plan time); all strings EN/PL/UK; E2E Gherkin covers toggle + range + pie interaction.

---

_Roadmap version: v1.1 milestone — generated 2026-05-11. Coverage: 126/126 v1.1 requirements mapped. Phase 7 rescoped 2026-05-30: 3-kind set (RESERVE_TOPUP, CONFIRM_DRAFT, CUSHION_BELOW_TARGET); TASK-05 dropped from v1.1; TASK-04 reassigned semantic to CUSHION_BELOW_TARGET. Phases 11–12 added 2026-06-28 (v1.2 Overview tab; split from a single spec — budget-side vs financial-wealth)._
