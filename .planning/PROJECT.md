...
## Current Milestone: v1.1 Budget Restructure

**Goal:** Replace multi-page workspace UI with single Excel-like Budget Detail Page; rename workspace→budget and account→wallet; drop income/transfer transaction kinds; reserves auto-compute per-category from past-month remainders; cushion as workspace-wide toggle; PWA + offline + web-push for tasks; dev DB nuked (no migration script).

**Target features:**

- Rename + schema migration (workspace→budget, account→wallet, transaction strip, wallet_type enum, cushion column, tasks table)
- Top-nav budget switcher dropdown (Personal/Shared groups + `+` aside button)
- Combined home page (per-budget cards + placeholder chart)
- Spendings tab (Excel-like categories × current-month grid with quick-entry, pen sliders, drag-reorder, `+` dashed column, arrow-key month nav, recurring drafts inline)
- Reserves tab (auto-computed per-category balances + reconciliation tasks against reserve-type wallets)
- Wallets tab (always-inline editable rows: name/currency/amount/type, `+ Add` row)
- Settings tab (name+currency lock-after-first-txn, cushion toggle, recurring rules CRUD, members for SHARED, danger zone)
- Recurring drafts as highlighted grid rows with confirm action
- Tasks queue (reserve mismatch, recurring due, stale wallet, month-end close) — top-banner with expand
- PWA + offline shell + web-push for task notifications
- i18n EN/PL/UK rewrite for new IA
- Onboarding wizard (name + currency + starter category templates)

**Reference:** `.planning/v1.1-SPEC.md` — full decision record from 2026-05-11 brainstorming.

**Out of scope for v1.1:** Investments domain · Insights/analytics dashboards (placeholder only) · Voice STT capture · LLM smart-category suggestions · Comparison/anonymizer · Email digest · Native mobile · Receipt OCR · Bank API integration.

**Legacy v1.0 status:** Phases 1–2 shipped (Foundations · Budgeting & FX). Legacy planned phases 3–6 (Reserve/Investments/Cushion · Tasks/Insights/Notifications · Onboarding/Comparison · Launch Hardening) frozen as not-built; their scope re-derived under v1.1. Archived to `.planning/archive/v1.0/`.
...
_Last updated: 2026-05-11 — v1.1 Budget Restructure milestone opened_
