...
| 260720-eml | 2026-07-20 | Email dual-send: real SMTP (Mailtrap live) + mailpit mirror via `MAILPIT_ENABLED`; reserved-domain guard so E2E (`@test.local`/`@example.*`) never hits the real provider; `SkipReservedDomainsEmailSender` + `FanOutEmailSender` (allSettled → mirror never lost to a flaky real leg); relaxed `SMTP_FROM` to allow display-name form (was crash-looping boot); override no longer pins api SMTP_* to mailpit. Live: real signup → mailpit verify email + Mailtrap real send to gmail                                                                                                                                                                                                                                                                                       | complete ✓ (live-verified)                                        | [PLAN](quick/20260720-email-mailpit-mirror/PLAN.md)                                  | [SUMMARY](quick/20260720-email-mailpit-mirror/SUMMARY.md)                                  |
| 260720-sav | 2026-07-20 | `savings` investment type: manual name/currency/starting/current, qty hidden (stored "1"), `%change=(current−starting)/starting` (reuses profitLossPct — zero new compute), own pie slice + Wallet icon + lime accent; `other` icon MoreHorizontal→Shapes. Rides the `broker` field-set relabeled Starting/Current. Migration 0065 (holding_type+ui_type CHECKs, journal idx 65). TDD across domain/contracts/types/icons/form/row/i18n(en·pl·uk) + @investments-wallet e2e. Live-verified on Travel + e2e green vs dev                                                                                                                                                                                                                                                      | complete ✓ (live-verified)                                        | [PLAN](quick/20260720-investment-savings-type/PLAN.md)                               | [SUMMARY](quick/20260720-investment-savings-type/SUMMARY.md)                               |
| 260721-pos | 2026-07-21 | Possessions wallet section (house/car/jewelry/…): new `possession` holdingType on the investments table + one nullable `icon` column (migration 0066, journal idx 66). Always-on section after investments; thin bespoke PossessionSheet/Row/Section (reuse WalletCustomizer icon-only via new `icons`+`showColor` props; 13 possession icons). Part of capitalization but EXCLUDED from the retirement pot — `HoldingsValuationPort.possessionsValueCents`, capitalization=wallets+investments+possessions, retirement W=cap−possessions (BDP + aggregate). Excluded from investment value/cost/pie. TDD domain/contracts/wealth/overview-cards/aggregate + web (possessions, aggregate-overview) + @possessions-wallet e2e. Live-verified vs dev (e2e green + screenshots) | complete ✓ (live-verified)                                        | [PLAN](quick/20260721-possessions-wallet/PLAN.md)                                    | [SUMMARY](quick/20260721-possessions-wallet/SUMMARY.md)                                    |
| 260729-bbj | 2026-07-29 | Spendings footer "Last spending added" no longer reports a draft-era timestamp: `latestSpendingCreatedAt` returns `max(confirmed_at)` instead of `max(created_at)` (unconfirmed drafts were already excluded, but a _confirmed_ recurring draft reported the worker's 06:00 generation time). TDD: new `seedDraftAt` helper + integration test (pending draft with newest created_at ignored; confirmed draft dates by its confirmation) — red 1 fail → green 9 pass on real Postgres                                                                                                                                                                                                                                                                                        | complete ✓ (built, not yet restarted — Infisical session expired) | [PLAN](quick/260729-bbj-spendings-footer-last-added-uses-confirm/260729-bbj-PLAN.md) | [SUMMARY](quick/260729-bbj-spendings-footer-last-added-uses-confirm/260729-bbj-SUMMARY.md) |
| 260812-dgf | 2026-08-12 | Wallets currency picker rendered 96px on desktop instead of 224px: the cell's Tailwind classes ended glued to a template interpolation (`md:w-[224px]${…}`), so the scanner never emitted the utility (zero `224px` in the served CSS) and the rich label's name span collapsed to 3px. Space before `${` at all 3 glue sites + tree-wide guard test (red on the 2 offenders → green). Measured 96px → 224px, name span 3px → 61px                                                                                                                                                                                                                                                                                                                                           | complete ✓                                                        | [PLAN](quick/260812-dgf-fix-currency-picker-width-on-desktop-wal/260812-dgf-PLAN.md) | [SUMMARY](quick/260812-dgf-fix-currency-picker-width-on-desktop-wal/260812-dgf-SUMMARY.md) |

| 260812-fya | 2026-08-12 | Money-forecast line explains itself: every day carries `available = opening + income − bills − plannedBurn + reserveCovered` (asserted per cell) and the tooltip reads it out, so the first cell no longer silently sits one day's burn below the "available to spend" card. Unconfirmed past-due occurrences — in neither the dated bills nor confirmed spend once their draft rolled `next_due_date` — are now listed on today's cell (informational, they move no cash). Pie centre says "N categories" once narrowed, still "All categories" when only Investments is dropped. Burn rule deliberately unchanged (plan − spent over days LEFT; passed days' unspent money does not expire) | complete ✓ | [PLAN](quick/260812-fya-forecast-tooltip-shows-the-spend-equatio/260812-fya-PLAN.md) | [SUMMARY](quick/260812-fya-forecast-tooltip-shows-the-spend-equatio/260812-fya-SUMMARY.md) |
| 260814-jfw | 2026-08-14 | Dependency refresh: every outdated package across 13 workspaces to latest stable — zod 3→4, nanoid 6, pino 10, motion 13, temporal-polyfill 1.0.4, libsodium-wrappers 0.8.4, recharts 3.10.1, Playwright 1.62.1 + playwright-bdd 9, jest-dom 7 (+ the `@testing-library/dom` peer it now requires), fast-check 4, lint-staged 17, dependency-cruiser 18, @typescript-eslint 8.67, the `@types/*`, CI Node 20→22. **TypeScript held at 5.9.3** (typescript-eslint throws on TS 7, killing the lint gate and `local/no-float-money`; TS 6 is beta-only) and **Next held at 16.2.12** (16.3.1 passes `next build` on the host but fails the image build — Bun "Expected CommonJS module to have a function wrapper" collecting page data for /icon.svg). Along the way: playwright-bdd 9's arity check caught a step definition discarding its captured argument; `@budget/budgeting` had no `typecheck` script, hiding 27 type errors from a CI gate — script added, all 27 fixed | complete ✓ | [PLAN](quick/260814-jfw-dependency-refresh/260814-jfw-PLAN.md) | [SUMMARY](quick/260814-jfw-dependency-refresh/260814-jfw-SUMMARY.md) |
| 260824-f2o | 2026-08-24 | Forecast strip month labels: `MIN_LABEL_PCT` 8 → 12. The 260823 guard sized the threshold from glyph width alone, ignoring the label's 8px lead-in and any trailing gap, so a segment could clear the check and still print on the divider — live on 24 Aug, where Sep turns on day 8 of 100 (8.08%) and "Aug" landed flush against its rule. One constant covers both ends (head room = distance to next open, tail room = `100 - pct`). The unit tests could never have caught this: happy-dom lays out nothing, so a percentage guess 2px short reads green. Added an `@overview @projection` E2E measuring real boxes — which **passed against a deliberately-rebuilt broken image** on its first run (`.sort()` lexicographic; "next divider" searched from the label's right edge, so an overlapping label skipped its own rule), and failed at **−2.82px** once corrected | complete ✓ | [PLAN](quick/260824-forecast-month-label-padding/PLAN.md) | [SUMMARY](quick/260824-forecast-month-label-padding/SUMMARY.md) |
