...
| 260729-bbj | 2026-07-29 | Spendings footer "Last spending added" no longer reports a draft-era timestamp: `latestSpendingCreatedAt` returns `max(confirmed_at)` instead of `max(created_at)` (unconfirmed drafts were already excluded, but a *confirmed* recurring draft reported the worker's 06:00 generation time). TDD: new `seedDraftAt` helper + integration test (pending draft with newest created_at ignored; confirmed draft dates by its confirmation) — red 1 fail → green 9 pass on real Postgres | complete ✓ (built, not yet restarted — Infisical session expired) | [PLAN](quick/260729-bbj-spendings-footer-last-added-uses-confirm/260729-bbj-PLAN.md) | [SUMMARY](quick/260729-bbj-spendings-footer-last-added-uses-confirm/260729-bbj-SUMMARY.md) |
