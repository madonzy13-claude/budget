  ...
  return async (input: {
    ...
    const loaded = await withTenantTx(
      ...
      async (txRaw) => {
        ...
        // POINT-IN-TIME predicates (limit effective ON one date), NOT a month range:
        // SCD-2 keeps category_limits non-overlapping at any instant, so an
        // equality-at-a-date join returns exactly ONE row per category. A range
        // predicate (effective_from <= monthEnd AND effective_to > monthStart) matches
        // BOTH sides of a mid-month limit change → duplicate rows → doubled budget.
        // `tl` = limit effective today; `nl` = limit effective at the 1st of next month.
