...
describe("deposit backdate supplement", () => {
  // A deposit entered 2026-03-01 but started earlier: buckets BEFORE creation get
  // its analytic value (+5000 here); buckets on/after already carry it (the cron
  // captured it / live point) → +0, so no double count.
  const depositBackdate =
    (): NonNullable<GetOverviewWealthDeps["depositBackdate"]> =>
    async () =>
    (dateISO: string) => (dateISO < "2026-03-01" ? 5000n : 0n);

  test("adds the supplement to pre-creation buckets (capitalization view)", async () => {
    const withDep = (
      await getOverviewWealth(deps({ depositBackdate: depositBackdate() }))({
        ...base,
        view: "capitalization",
      })
    )._unsafeUnwrap();
    ...
    // First real snapshot bucket (Jan 5) rises by the supplement.
    const firstNonZero = (d: OverviewWealthDTO) =>
      Number(d.series.find((p) => Number(p.value_cents) !== 0)!.value_cents);
    expect(firstNonZero(withDep) - firstNonZero(without)).toBe(5000);

    // The live current point (March, on/after creation) is unchanged — no double
    // count where the snapshot already carries the deposit.
    expect(withDep.series.at(-1)!.value_cents).toBe(
      without.series.at(-1)!.value_cents,
    );
  });

  test("applies to the investments view too (a deposit is an investment)", async () => {
    const withDep = (
      await getOverviewWealth(deps({ depositBackdate: depositBackdate() }))({
        ...base,
        view: "investments",
