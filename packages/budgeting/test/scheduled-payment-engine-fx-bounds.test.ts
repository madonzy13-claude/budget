/**
 * scheduled-payment-engine-fx-bounds.test.ts — T-02-WORKER-FX mitigation unit test.
 *
 * `computeScheduledFx` is the FX-computation helper used by the scheduled
...
function input(overrides: Partial<ScheduledFxInput> = {}): ScheduledFxInput {
  return {
    ruleCurrency: "USD",
    budgetCurrency: "USD",
...
describe("computeScheduledFx (T-02-WORKER-FX)", () => {
  test("same currency: rate=1, converted=original, no FX call (path identity)", async () => {
    ...
    const out = await computeScheduledFx(input({ fxProvider }));
  ...
  test("cross-currency: FX provider invoked, converted reflects rate", async () => {
    const out = await computeScheduledFx(
      input({
        ruleCurrency: "EUR",
        budgetCurrency: "USD",
        amountOriginalCents: "10000",
