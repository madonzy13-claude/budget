/**
 * scheduled-payment-domain.test.ts — Domain unit tests for ScheduledPayment
 * Tests: computeNextDueDate, cadence math (Pitfall 6 month-end), domain invariants
 *
 * v1.1 (Plan 02-02): ScheduledPayment constructor updated — no accountId, no kind, yearlyMonth added.
 */
import { describe, test, expect } from "bun:test";
import { Temporal } from "temporal-polyfill";
import { ScheduledPayment } from "../src/domain/scheduled-payment";
import { nextDueDateAfter } from "../src/domain/cadence";

describe("ScheduledPayment domain", () => {
  describe("computeNextDueDate — MONTHLY", () => {
    test("anchor=31: Jan 31 → Feb 28 (Pitfall 6 month-end)", () => {
      const rule = new ScheduledPayment(
        "rule-1",
        "tenant-1",
        null,
        "500",
