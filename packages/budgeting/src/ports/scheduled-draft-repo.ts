/**
 * scheduled-draft-repo.ts — Port interface for scheduled draft persistence.
 *
 * v1.1 (Phase 2, Plan 02-02):
 *   Drafts are now expense_ledger rows with confirmed_at IS NULL.
 *   The separate scheduled_drafts table is DROPPED by migration 0013 (02-01).
...
export interface ScheduledDraftRepo {
  /** Find by id (RLS-scoped). Returns null if not found or wrong tenant. */
  findById(tenantId: string, draftId: string): Promise<ScheduledDraftRow | null>;

  /** List pending drafts (confirmed_at IS NULL) for tenant ordered by transaction_date ASC. */
  listPending(tenantId: string): Promise<ScheduledDraftRow[]>;
   ...
   * Only touches rows WHERE scheduled_payment_id=$1 AND confirmed_at IS NULL AND transaction_date >= CURRENT_DATE.
   * Preserves draft.id and UNIQUE (scheduled_payment_id, transaction_date) invariant.
