/**
 * confirm-scheduled-draft.ts — Confirm a draft (expense_ledger row with confirmed_at IS NULL).
 *
 * v1.1 (Phase 2, Plan 02-02):
 *   Drafts are now expense_ledger rows with confirmed_at IS NULL (scheduled_payment_id set).
...
export interface ConfirmScheduledDraftDeps {
  /** Phase 7 (D-PH7-09): auto-resolve the CONFIRM_DRAFT task on confirm. */
  taskRepo?: TaskRepo;
  /** 05-17: confirming a scheduled draft flips confirmed_at → the row becomes
   *  counted spend → can overspend a category → draws reserve → shifts surplus.
   *  Refresh RESERVE_TOPUP after the confirm. Optional + gated; best-effort
   *  own-tx after the confirm commits (sweep is the backstop). */
...
export interface ConfirmScheduledDraftInput {
  tenantId: string;
  draftId: string;
  actorUserId: string;
}
...
export function confirmScheduledDraft(deps: ConfirmScheduledDraftDeps = {}) {
  return async (
    input: ConfirmScheduledDraftInput,
    ...
    const r = await withTenantTx(
      ...
      async (tx) => {
        ...
        SELECT id, confirmed_at, deleted_at, scheduled_payment_id, transaction_date
      ...
      `);
        ...
        await writeOutbox(tx, {
          ...
          eventType: "budgeting.scheduled.confirmed",
          payload: {
    ...
    // 05-17: refresh RESERVE_TOPUP off the live engine surplus after a successful
    // confirm (the now-confirmed draft is counted spend → may draw reserve).
    // Best-effort own-tx; never fails the confirm — the sweep reconverges.
    if (
      r.isOk() &&
      deps.taskRepo &&
      ...
      if (recomputeR.isErr()) {
        console.error(
          "[confirm-scheduled-draft] reserve-topup recompute failed:",
          recomputeR.error,
        );
      }
    }
