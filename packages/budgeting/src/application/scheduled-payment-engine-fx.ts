/**
 * scheduled-payment-engine-fx.ts — FX computation helper for the scheduled draft engine.
 *
 * Encapsulates the cross-currency branch + rate-bounds check used by both the
 * worker handler (`apps/worker/src/handlers/scheduled-payment-engine.ts`) and the
 * application-layer engine (`packages/budgeting/src/application/scheduled-payment-engine.ts`).
 *
...
export interface ScheduledFxInput {
  ruleCurrency: string;
...
export interface ScheduledFxResult {
  fxRate: string;
  fxAsOf: string;
  amountConvertedCents: string;
}

export async function computeScheduledFx(
  input: ScheduledFxInput,
): Promise<ScheduledFxResult> {
