/**
 * scheduled-monthly-normalize.ts — pure 4-cadence → monthly-cents normalizer (11-04).
 *
 * scheduled_payments.cadence CHECK allows DAILY/WEEKLY/MONTHLY/YEARLY (Pitfall 2), so
 * all four must map to a comparable monthly figure for the per-category scheduled
...
export function scheduledMonthlyNormalize(
  amountCents: bigint,
  cadence: Cadence,
): bigint {
  switch (cadence) {
