/**
 * create-scheduled-payment.ts — Create a scheduled rule use case.
 *
...
export interface CreateScheduledPaymentResult {
  ruleId: string;
}
...
export function createScheduledPayment(deps: {
  ...
  ): Promise<Result<CreateScheduledPaymentResult, Error>> => {
    // Past first_due_date is now allowed — the user often seeds a
    // rule from a past start (e.g. a salary that began last month) so
    // the scheduled engine generates the correct stream of drafts
    // from that anchor onwards. The scheduled engine's catch-up logic
    ...
    const r = await withTenantTx(
      ...
      `);
        ...
        await writeOutbox(tx, {
          ...
          aggregateId: ruleId,
          eventType: "budgeting.scheduled.rule.created",
        ...
        // `computeScheduledFx` to fetch the rate-as-of the due date.
