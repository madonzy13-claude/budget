...
export function getOverviewWealth(deps: GetOverviewWealthDeps) {
  return async (
    ...
    try {
      ...
      // dynamics — INTRA-period change (260726): each bar is the period's OWN
      // first→last movement (start-of-period value to end-of-period value), NOT the
      // step from the previous period. So EVERY period gets a bar — including the
      // first (the opening year/month) — and a bar answers "how much did wealth move
      // DURING this period". Group the in-range snapshots by the dyn bucket (day /
      // month / year) and take the first + last value within each.
      const dynBucket = dynamicsBucketOf(input.from, input.to);
      const dynFirst = new Map<string, bigint>();
      const dynLast = new Map<string, bigint>();
      const dynOrder: string[] = [];
      const pushDyn = (label: string, v: bigint) => {
        if (!dynFirst.has(label)) {
          dynFirst.set(label, v);
          dynOrder.push(label);
        }
        dynLast.set(label, v);
      };
      // Chronological order → correct first/last per bucket. Daily reads intra-day
      // movement from the (hourly) rows; monthly/yearly span the period's real ends.
      const chronoRows = [...rows].sort(
        (a, b) => a.captured_at.getTime() - b.captured_at.getTime(),
      );
      for (const r of chronoRows)
        pushDyn(dynLabelOf(r.captured_at, dynBucket), pickAdj(r));
      if (liveInRange) pushDyn(dynLabelOf(now, dynBucket), pickAdj(live));
      const dynamics = dynOrder.map((label) => {
        const first = dynFirst.get(label)!;
        const last = dynLast.get(label)!;
        return {
          label,
          pct:
            first === 0n ? null : (Number(last - first) * 100) / Number(first),
          delta_cents: (last - first).toString(),
        };
      });
