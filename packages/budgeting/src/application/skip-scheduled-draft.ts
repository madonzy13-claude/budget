/**
 * skip-scheduled-draft.ts — Skip a pending draft (soft-delete expense_ledger row).
 *
...
export interface SkipScheduledDraftDeps {
  /** Phase 7 (D-PH7-10): auto-resolve the CONFIRM_DRAFT task on skip. */
  taskRepo?: TaskRepo;
}

export interface SkipScheduledDraftInput {
  tenantId: string;
  draftId: string;
  actorUserId: string;
}

export function skipScheduledDraft(deps: SkipScheduledDraftDeps = {}) {
  return async (
    input: SkipScheduledDraftInput,
    ...
    const r = await withTenantTx(
      ...
      `);
        ...
        await writeOutbox(tx, {
          ...
          eventType: "budgeting.scheduled.skipped",
