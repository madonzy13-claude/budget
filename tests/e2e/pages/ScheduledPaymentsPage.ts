...
/**
 * ScheduledPage — Page Object for /[locale]/(app)/scheduled
 *
...
export class ScheduledPage {
  ...
  async goto(locale = "en"): Promise<void> {
    await this.page.goto(`/${locale}/scheduled`);
  }

  // ── Top-of-page CTA ──────────────────────────────────────────────────────

  addRuleButton(): Locator {
    return this.page.getByRole("button", { name: /add scheduled rule/i });
  }
  ...
  async saveRule(): Promise<void> {
    ...
    // Form reloads page after save (see scheduled-page-client.tsx onSaved).
