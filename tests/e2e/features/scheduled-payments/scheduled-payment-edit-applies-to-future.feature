...
Feature: Editing a scheduled rule with "Also apply to future occurrences" pre-checked
  ...
  Scenario: User edits a rule's amount; pre-checked checkbox propagates to the existing PENDING draft
    Given I have a monthly scheduled rule "Rent" of 1500 USD anchored to day 1 in category "Rent"
    And the engine has generated a PENDING draft for "Rent" at 1500 USD
    When I open the Scheduled page
    And I open the edit form for the scheduled rule "Rent"
    Then the "Also apply to future occurrences" checkbox is checked
    When I change the scheduled rule amount to "1600"
    And I save the scheduled rule
    Then I see a scheduled rule in the list with amount "1600"
