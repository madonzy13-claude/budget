/**
 * workspace-settings.ts — /workspace-settings route
 * Budget mode toggle (D-04-e): NORMAL|CUSHION SCD-2 history.
 */
import { Hono } from "hono";
import type { BootedDeps } from "../boot";

export function createWorkspaceSettingsRoute(deps: BootedDeps) {
  ...
  // POST /workspace-settings/budget-mode — set budget mode
  app.post("/budget-mode", async (c) => {
    const { setBudgetModeSchema } = await import(
    ...
    const r = await deps.budgeting.toggleBudgetMode({
      tenantId,
      workspaceId: tenantId, // workspace = tenant in phase 2
