/**
 * scheduled-drafts-dismiss.test.ts — Integration tests for
...
async function buildApp(userId: string, tenantId: string) {
  const { createScheduledPaymentsRoute } = await import("../../src/routes/scheduled-payments");
  ...
  app.route("/budgets/:budgetId/scheduled-payments", createScheduledPaymentsRoute(deps));
  return app;
}

describe("POST /budgets/:budgetId/scheduled-payments/drafts/:draftId/dismiss", () => {
  let fix: Fixture;
  ...
  it("dismisses a pending draft → 204", async () => {
    ...
    const res = await app.request(
      `/budgets/${fix.tenantId}/scheduled-payments/drafts/${draftId}/dismiss`,
      { method: "POST" },
    );
  ...
  it("returns 404 for non-existent draft", async () => {
    ...
    const app = await buildApp(fix.userId, fix.tenantId);
    const res = await app.request(
      `/budgets/${fix.tenantId}/scheduled-payments/drafts/${fakeDraftId}/dismiss`,
      { method: "POST" },
    );
  ...
  it("tenant-leak: wrong budgetId → 403 (D-PH4-E3)", async () => {
    ...
    const app = await buildApp(fix.userId, fix.tenantId);
    const res = await app.request(
      `/budgets/${otherTenantId}/scheduled-payments/drafts/${draftId}/dismiss`,
      { method: "POST" },
    );
