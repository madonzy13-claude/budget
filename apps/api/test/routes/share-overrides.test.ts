...
async function createTestUser(): Promise<{ userId: string; tenantId: string }> {
  ...
  try {
    ...
    await client.query(`INSERT INTO tenancy.workspaces (id, slug, name, kind, default_currency, owner_user_id, member_count, created_at) VALUES ($1, $2, 'SO WS', 'PRIVATE', 'EUR', $3, 1, now())`, [tenantId, `ws-so-${tenantId.slice(0, 8)}`, userId]);
