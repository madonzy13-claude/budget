/**
 * scheduled-drafts.test.ts — Integration tests for /scheduled-drafts routes.
...
async function seedDraft(
  ...
  try {
    ...
    // Drafts are expense_ledger rows with confirmed_at IS NULL (scheduled_drafts table dropped in 02-01)
    await client.query(
      `INSERT INTO budgeting.expense_ledger
         (id, tenant_id, budget_id, transaction_date, amount_original_cents, currency_original,
          amount_converted_cents, fx_rate, fx_as_of, kind, scheduled_payment_id, confirmed_at, created_at, updated_at)
...
async function buildApp(userId: string, tenantId: string) {
  const { createScheduledDraftsRoute } =
    await import("../../src/routes/scheduled-drafts");
  ...
  app.route("/scheduled-drafts", createScheduledDraftsRoute(deps));
  return app;
}
...
describe("/scheduled-drafts", () => {
  ...
  it("GET lists pending drafts", async () => {
    ...
    const res = await app.request("/scheduled-drafts");
  ...
  it("POST /:id/confirm → 201 with ledgerId", async () => {
    ...
    const app = await buildApp(testUserId, testTenantId);
    const res = await app.request(`/scheduled-drafts/${draftId}/confirm`, {
      method: "POST",
      headers: {
  ...
  it("POST /:id/confirm twice → second is 409 already_confirmed", async () => {
    ...
    const app = await buildApp(testUserId, testTenantId);
    const r1 = await app.request(`/scheduled-drafts/${draftId}/confirm`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Idempotency-Key": crypto.randomUUID(),
    ...
    const r2 = await app.request(`/scheduled-drafts/${draftId}/confirm`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Idempotency-Key": crypto.randomUUID(),
  ...
  it("POST /:id/skip → 204", async () => {
    ...
    const app = await buildApp(testUserId, testTenantId);
    const res = await app.request(`/scheduled-drafts/${draftId}/skip`, {
      method: "POST",
  ...
  it("POST /:id/edit-confirm with new amount → 201", async () => {
    ...
    const app = await buildApp(testUserId, testTenantId);
    const res = await app.request(`/scheduled-drafts/${draftId}/edit-confirm`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Idempotency-Key": crypto.randomUUID(),
  ...
  it("POST /:id/confirm on non-existent draft → 404", async () => {
    ...
    const app = await buildApp(testUserId, testTenantId);
    const res = await app.request(`/scheduled-drafts/${fake}/confirm`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Idempotency-Key": crypto.randomUUID(),
