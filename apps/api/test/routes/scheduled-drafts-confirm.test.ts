/**
 * scheduled-drafts-confirm.test.ts — Integration tests for
...
describe("POST /budgets/:budgetId/scheduled-payments/drafts/:draftId/confirm", () => {
  let fix: Fixture;
  ...
  it("returns 404 for non-existent draft", async () => {
    ...
    const app = await buildApp(fix.userId, fix.tenantId);
    const res = await app.request(
      `/budgets/${fix.tenantId}/scheduled-payments/drafts/${fakeDraftId}/confirm`,
      { method: "POST" },
    );
  ...
  it("tenant-leak: wrong budgetId → 403 (D-PH4-E3)", async () => {
    ...
    const app = await buildApp(fix.userId, fix.tenantId);
    const res = await app.request(
      `/budgets/${otherTenantId}/scheduled-payments/drafts/${draftId}/confirm`,
      { method: "POST" },
    );
