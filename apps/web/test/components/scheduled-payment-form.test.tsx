/**
 * scheduled-payment-form.test.tsx — Vitest+RTL tests for ScheduledPaymentForm.
 *
...
vi.mock("next-intl", () => ({
  ...
  useTranslations: (_ns: string) => (key: string) => {
    const map: Record<string, string> = {
      "rule.title": "New scheduled rule",
      "rule.editTitle": "Edit scheduled rule",
...
const { ScheduledPaymentForm } =
  await import("../../src/components/budgeting/scheduled-payment-form");

describe("ScheduledPaymentForm — applyToFuture is forced on (UAT-7 retest)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe("ScheduledPaymentForm — Category mandatory (UAT-7 retest)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe("ScheduledPaymentForm — frequency tiles (UAT-7)", () => {
...
describe("ScheduledPaymentForm — edit-mode field prefill (UAT-7 retest)", () => {
  ...
  it("syncs the Amount input when initialValues changes between renders (no remount)", () => {
    ...
    const { rerender } = render(<ScheduledPaymentForm {...props} />);
...
describe("ScheduledPaymentForm — Category dropdown (UAT-7 retest)", () => {
...
describe("ScheduledPaymentForm — optional last date (260721)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe("ScheduledPaymentForm — WEEKLY weekday picker (UAT-7)", () => {
