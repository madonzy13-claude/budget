...
export interface ColumnHeaderProps {
  ...
  /** Called when the archived column's revert (unarchive) icon is clicked.
   *  No confirm dialog — restores the category to a normal editable column. */
...
export function ColumnHeader({
  ...
  // Reveal classes shared by every action button: hidden until tapped
  // (revealed) or desktop hover anywhere in the header (group-hover — the
  // `group` lives on the header ROOT, so all 5 rows reveal, 260611-vuo).
  ...
  return (
    ...
    >
      {/* Header ROOT (260611-vuo): carries the reveal hook ref + `group` so
          hover/tap on ANY of the 5 rows reveals the action cluster, and a
          pointerdown outside the whole header closes it. */}
      <div
        ref={ref}
        data-testid="column-header-root"
        className="group flex w-full flex-col"
      >
        {/* Row 1: Grip + Name + overlay action cluster.
          UAT round 20: cell itself uses the default cursor; pointer
          lives only on the action buttons (the actual click targets).
          260611-vuo BUG1: the pen/trash/revert sit in an ABSOLUTE cluster
          pinned right, so they never reserve inline width — the name span
          truncates at the FULL header width on non-archived columns. */}
        <div
          ...
          className={cn(
            ...
            "relative flex w-0 min-w-full min-h-[44px] items-center gap-1 px-2 py-2",
        ...
        >
          ...
          {/* "archived" tag — inline + shrink-0 so the row stays single-line and
            aligned with every other column's name row. Only archived columns
            reserve this slot; normal columns give the name the full width. */}
          {archived && (
            <span className="shrink-0 rounded bg-[var(--surface-elevated-dark)] px-1 py-0.5 text-[9px] lowercase tracking-wider text-[#7A7C7F]">
              {t("archived")}
            </span>
          )}
          {/* Overlay action cluster (260611-vuo BUG1): absolute, pinned right.
            Solid bg ONLY while visible (revealed or group-hover) so the
            invisible cluster never paints over the name's tail. */}
          <div
            data-testid="column-header-actions"
            className={cn(
              "absolute right-1 top-1/2 -translate-y-1/2 z-10 flex items-center gap-0.5 rounded",
              revealed
                ? archived
                  ? "bg-[#14181D]"
                  : "bg-[var(--surface-card-dark)]"
                : archived
                  ? "group-hover:bg-[#14181D]"
                  : "group-hover:bg-[var(--surface-card-dark)]",
            )}
          >
            ...
            {/* Archived → revert (unarchive) — restores the category, NO confirm. */}
            {archived && (
              <button
                type="button"
                data-testid={`column-header-revert-${category.name.toLowerCase()}`}
                onClick={(e) => {
                  e.stopPropagation();
                  onUnarchive?.(category.id);
                  setRevealed(false);
                }}
        ...
        {/* Row 2: Planned / Cushion — NO double-click (D-PH4-INT4).
          260611-vuo FEATURE3: tap toggles the action reveal (column-wide). */}
        <div
          onClick={toggleReveal}
          onDoubleClick={handleDoubleClick}
                ...
                <>
                  ...
                  >
                    ...
                    {reserveExcluded
                      ? "—"
                      : centsToBare(
                          summary.reserveAvailableCents ?? "0",
                          locale,
                        )}
                  </span>
                </>
