...
/**
 * scheduled-payments-section.tsx — D-03 + UAT-Phase6-Test7 retest
 *
 * Renders ScheduledPaymentsList inline. Add CTA opens the scheduled-payment
 ...
 *   - Delete fires DELETE /api/budgets/:id/scheduled-payments/:id behind
...
export function ScheduledSection({
  ...
  const tRec = useTranslations("budgeting.scheduled");
  ...
  const { data: fetchedRules } = useQuery<ScheduledPaymentListItem[]>({
    queryKey: ["scheduled-payments", budgetId],
    queryFn: async () => {
      if (!budgetId) return [];
      const res = await fetch(`/api/budgets/${budgetId}/scheduled-payments`, {
        credentials: "include",
        headers: { "X-Budget-ID": budgetId },
      });
      if (!res.ok) return [];
      const data = (await res.json()) as { rules?: ScheduledPaymentListItem[] };
      return data.rules ?? [];
    },
    enabled: !!budgetId,
  ...
  const handleSaved = () => {
    setSheetOpen(false);
    setEditRuleId(null);
    qc.invalidateQueries({ queryKey: ["scheduled-payments", budgetId] });
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await clientApiWrite(
        `/budgets/${budgetId}/scheduled-payments/${pendingDeleteId}`,
        {
          method: "DELETE",
          credentials: "include",
          headers: { "X-Budget-ID": budgetId },
