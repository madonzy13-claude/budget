...
const { runBudgetingReconciliation } = await import("../../src/handlers/budgeting-reconciliation");
...
async function corruptProjection(tenantId: string, categoryId: string, monthStart: string, value: string) {
  const pool = new Pool({ connectionString: DB_URL });
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
...
describe("budgeting-reconciliation handler", () => {
  test("aggregates repaired+alerted across multiple tenants", { timeout: 30000 }, async () => {
